...
# LittleSynergy — Skill di Manutenzione v1.5 (9 luglio 2026)

> **v1.5:** **LINK FLOOR ≥2** (`ensureDoterraBridge`) + **difesa STRUTTURALE oli avoid-list** (rimossi dal `link_expert`, non solo vietati dal prompt) + catalogo kids-curato (§2.12). Lezione: il catalogo NON è il vincolo sul numero di link (EN aveva 140 slug e un articolo a 1 link); e la regola prompt da sola NON regge (7 link su oli avoid erano passati).
...
### 2.12 — LINK FLOOR ≥2 + DIFESA STRUTTURALE OLI AVOID-LIST (v1.5)
**Diagnosi da non ripetere:** il numero di link in-body NON dipende dalla dimensione del `link_expert`. EN aveva **140 slug** e produceva comunque un articolo a **1 link**; le lingue con 29 slug stavano a 4-9. → **Il vincolo è l'enforcement del minimo, non il catalogo.** Prima di "seminare 100-150 slug", misura: `link_expert` per lingua + min/avg/max link negli articoli published.
- **`ensureDoterraBridge(content, linkExpert, worldLinkUrl)`** in `generate-article` (dopo `stripEmDashes`): se i link doTERRA sono <2, **linkifica menzioni testuali di prodotti SICURI già presenti nel corpo**. Non inventa frasi, non linka mai un olio avoid-list, non tocca testo già dentro un markdown link. **World-link (pt/ja/ar) esclusi** (il corpo è già instradato al gateway da `sanitizeProductUrls`). Retro: `scripts/retro-link-floor.mjs`.
- **Prompt LINK RULES:** floor ≥2 esplicito ("mai zero, mai uno"), **anchor = nome prodotto NATIVO** (mai inglese), + **eccezione FALLBACK**: un olio in avvertenza NON si linka nemmeno al fallback URL, si nomina in **testo semplice**. «Linkare = avallare».
- **🚨 DIFESA STRUTTURALE (la lezione):** la regola prompt §2.8 da sola **NON ha retto** — 7 link su oli avoid-list erano passati in 4 articoli published generati DOPO la regola. Fix: **gli oli avoid-list (peppermint/eucalyptus/rosemary/wintergreen/cinnamon/clove/oregano/thyme) sono stati RIMOSSI dal `link_expert`** di tutte le lingue shop (en −11, es −15, −2 su ognuna delle 6 nuove). Ora il modello non ha proprio l'URL. **Tripla difesa: prompt §2.8 + eccezione fallback + link_expert senza avoid-oil.** Tool: `scripts/expand-link-expert-kids.mjs` (rimozione + espansione), `scripts/strip-avoid-links.mjs` (bonifica retro).
- **Catalogo kids-curato, NON 100-150 indiscriminati:** per un brand bambini espandere significa aggiungere prodotti *adatti* (doTERRA **Kids Collection** — `doterra-calmer-oil`, `kids-collection-kit/enrolment` — blend emotivi Adaptiv/Cheer/Console/Forgive/Peace, oli gentili: arborvitae, petitgrain, siberian-fir, sandalwood, helichrysum, neroli/rose touch), MAI integratori adulti né oli avoid-list. Fonte: `doterra-knowledge-master.json` (repo soloseo, read-only) — ⚠️ i suoi `seeded_anchors` sono **inglesi placeholder** per fr/pt/ro/it/nl/pl: gli anchor nativi vanno scritti a mano. Stato: en 129, es 91, fr 48, it/nl/ro/pl 47, de 38.
- **⚠️ GAP NOTO (da decidere):** il regex avoid-list copre gli oli SINGOLI, non i **blend che li contengono** (Deep Blue→wintergreen/peppermint, doTERRA Air→eucalyptus/peppermint, ZenGest→peppermint, On Guard→cinnamon/clove/eucalyptus/rosemary). Su un brand bambini valutare se rimuoverli dal `link_expert`.
- **E2E di verifica:** genera 1 draft su un tema che nomina gli oli da evitare (es. "beruhigende ätherische Öle für Kinder") → attesi ≥3 link, **0 link su avoid**, oli da evitare **nominati in testo semplice**.
