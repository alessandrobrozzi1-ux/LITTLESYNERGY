...
**cost_log:** l'insert è `await` (non `void`/fire-and-forget: su Vercel il void perdeva la riga prima del teardown → cost_log vuota). `cost_usd` = **tariffe UFFICIALI DeepSeek v4-pro** (USD/1M): input **0.27**, cache-hit **0.07**, output **1.10**. ⚠️ **Su DeepSeek `input_tokens` e `cache_read` sono pool SEPARATI** (NON annidati come Anthropic): **niente sottrazione**, somma dei 3 termini `(input*0.27 + cacheRead*0.07 + output*1.10)/1M`. **Il caching DeepSeek è automatico server-side**: MISURATO `cache_read=7552/6400` sui draft (>0 anche senza `cache_control`, il system-prompt grande viene cachato → input scende a ~100) → il termine cache è essenziale, NON toglierlo. E2E: ~$0.00245/articolo, combacia col balance reale ~$0.003. (Le mie prime tariffe 0.6/3 senza cache davano $0.0062 = 2×+ il reale — sbagliate.)
...
- **NICHE_MODIFIER (guard L3):** senza la regex per una lingua, `hasNicheModifier` ritorna `true` (guard SPENTO) → il cron genererebbe keyword non-kids. Aggiungere SEMPRE la regex per ogni nuova lingua in `lib/keyword-scorer.ts`.
- **FILTRO ETÀ (in `hasNicheModifier`, tutte le lingue):** `EXPLICIT_AGE` scarta le keyword con **numero+unità d'età** ("niños de 2 años", "6 meses", "生後6ヶ月", «6 أشهر») → non trascinano il numero nel titolo (brand bambini). Richiede numero+unità, quindi "top 5 oli"/"3 aceites" passano. Un articolo-titolo con l'età (topic-driven da ricerca reale, FAQ cauta) è un'eccezione da NON far diventare pattern; se già published, resta (non è una soglia-dose inventata — è il caso che il cervello distingue). Latine: `\b(stem…)` con **boundary iniziale aperto** (niente `\b` finale) = diacritic-proof. CJK/arabo: **substring senza `\b`** (es. ja `/(子ども|赤ちゃん|ママ|…)/`, ar `/(أطفال|رضّع|أمهات|…)/`, evitare stem troppo corti tipo bare `أم`).
...
- **⚠️ Quirk H1 mancante:** DeepSeek a volte omette la riga `# Titolo` e parte dalla byline → 0 H1 (SEO rotto), a volte un articolo esce **malformato** (0 heading, wall-of-text). **H1 ENFORCEMENT** in `generate-article` (dopo la finalizzazione del titolo): se il corpo non inizia con `# `, antepone `# ${title}`. I malformati (0 sezioni) vanno **rigenerati** (l'H1 da solo non basta). Retro `scripts/retro-seo-h1-meta.mjs`.
- **⚠️ Quirk `### FAQ` (H3) invece di `## FAQ` (H2)** (trovato da Creator-Garden): DeepSeek a volte mette la sezione FAQ a H3+ → check e schema FAQPage non la vedono. **`normalizeFaqHeading`** in `generate-article` (dopo `stripEmDashes`): promuove a `##` SOLO la riga del heading di SEZIONE FAQ, mai le domande interne (`### …?`). Idempotente. Regex **`FAQ_SECTION_PHRASE`** = "contains" (non full-match, per catturare varianti come **`Foire aux Questions (FAQ)`**) sulle frasi multi-parola native (Frequently Asked/Foire aux Questions/Fréquentes/Preguntas Frecuentes/Fragen/vragen/pytania/質問/`الأسئلة الشائعة` — ⚠️ arabo con `ال` opzionale su **entrambe** le parole) + bare `FAQ`/`(FAQ)` a parte. Misurato: 6+1/63 published avevano la FAQ a ### (tutte DeepSeek; il fr `Foire aux Questions (FAQ)` sfuggiva al primo regex).
...
- **SEO deterministico (post-processing generate-article):** **META CAP 160** (Google tronca oltre; taglia a confine di parola, prompt HARD CAP) · **H1 enforcement** (vedi §2.13) · **internal-link soglia `findRelatedArticles` = 0.35** (era 0.4: nicchia stretta oli-bambini, 0.4 lasciava ~44% senza link interno; 0.35 aumenta la copertura restando in-topic; fetch 3, prompt usa 1-2). Audit periodico: H1/meta/FAQ/foto/link-density per lingua (0 no-H1, 0 meta>160 al 07-19).
