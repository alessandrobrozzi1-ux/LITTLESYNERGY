...
- **⚠️ Quirk `### FAQ` (H3) invece di `## FAQ` (H2)** (trovato da Creator-Garden): DeepSeek a volte mette la sezione FAQ a H3+ → check e schema FAQPage non la vedono. **`normalizeFaqHeading`** in `generate-article` (dopo `stripEmDashes`): promuove a `##` SOLO la riga del heading di SEZIONE FAQ (frase nativa per lingua: `FAQ_SECTION_HEADING` = Fragen/vragen/pytania/Fréquentes/質問/`الأسئلة الشائعة`… ⚠️ arabo con `ال` opzionale su entrambe le parole), mai le domande interne (`### …?`). Idempotente. Retro `retro-strip-title-colon`-style. Misurato: 6/63 published avevano la FAQ a ### (tutte da generazione DeepSeek).
