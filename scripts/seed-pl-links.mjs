/**
 * Seed PL Link Expert — shop.doterra.com/PL/pl_PL/shop/ (Pattern 1, EU)
 * Brand ID: 22be89f9-8297-45eb-8a56-e4c0329d79f2
 ...
 * Idempotente: salta full_url già presenti per PL.
 */
...
const BRAND_ID = '22be89f9-8297-45eb-8a56-e4c0329d79f2'
const RO = '97933a00-3604-464a-92d6-e6ea8175cbc1'
const OID = '15957920'
const B = 'https://shop.doterra.com/PL/pl_PL/shop'
...
console.log(`PL — candidati EU: ${candidates.length} | già presenti: ${have.size} | da inserire: ${toInsert.length}`)
if (toInsert.length) {
...
const { data: allPl } = await supabase.from('link_expert').select('full_url').eq('brand_id', BRAND_ID)
const uniq = new Set((allPl ?? []).map(e => e.full_url)).size
console.log(`✅ Link Expert PL: ${count} | unici: ${uniq} ${uniq === count ? '✅ no doppioni' : '❌ DOPPIONI'} | es: ${candidates.slice(0, 5).map(titel).join(', ')}`)
