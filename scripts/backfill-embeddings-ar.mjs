...
// AR articles include drafts at launch — embed ALL so internal linking is ready once published.
const { data: articles } = await supabase.from('articles')
  .select('id, brand_id, title, meta_description, content_markdown, status')
  .eq('brand_id', AR).order('created_at', { ascending: true })

console.log(`Backfill embeddings AR — ${articles?.length ?? 0} articoli (tutti gli status)\n`)
let ok = 0
...
const { count } = await supabase.from('article_embeddings').select('*', { count: 'exact', head: true }).eq('brand_id', AR)
console.log(`\n✅ Embeddings AR: ${count} (${ok} processati ora)`)
